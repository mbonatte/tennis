...
class BounceDetector:
    ...
    def prepare_features(self, x_ball, y_ball):
        ...
        colnames_x = ['x_diff_{}'.format(i) for i in range(1, NUM_FEATURE_FRAMES)] + \
                     ['x_diff_inv_{}'.format(i) for i in range(1, NUM_FEATURE_FRAMES)] + \
                     ['x_div_{}'.format(i) for i in range(1, NUM_FEATURE_FRAMES)]
        
        colnames_y = ['y_diff_{}'.format(i) for i in range(1, NUM_FEATURE_FRAMES)] + \
                     ['y_diff_inv_{}'.format(i) for i in range(1, NUM_FEATURE_FRAMES)] + \
                     ['y_div_{}'.format(i) for i in range(1, NUM_FEATURE_FRAMES)]
    ...
    def draw_bounce(court_imgs, bounces, ball_track, homography_matrices):
        width_minimap = 166
        height_minimap = 350
        for i in bounces:
            if homography_matrices[i] is not None:
                ...
                court_imgs[i] = cv2.circle(court_img, (int(ball_point[0, 0, 0]), int(ball_point[0, 0, 1])),
