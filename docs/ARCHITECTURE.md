...
Completed jobs may have one optional `court_calibration` record. It contains four user-clicked outer doubles-court corners on one source frame and derives a static image-to-court homography. The correction is stored separately from the model artifact, can be removed, and is applied only when creating subsequent render outputs; it replaces model homographies and projected court keypoints for the whole fixed-camera video. Existing outputs remain immutable. It deliberately does not rewrite historical bounce calls, statistics, or results JSON; those are a later recalculation feature. Videos with camera changes need future keyframe calibration support.
