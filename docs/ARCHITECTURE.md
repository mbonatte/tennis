...
Uploads stream into `data/jobs/<uuid>/input/source.<ext>`. FFprobe validates content, codec, duration, frame rate, and resolution before a database row is queued. The worker scans exact readable frame count, then runs each enabled model as a separate bounded pass. A job owns one model/tracker instance per stage; stage objects close before the next model family loads. Compact per-frame results use zero-based source indexing. TrackNet alone adds two private overlap frames and trims their outputs before concatenation. Whole-track ball post-processing follows raw inference. A final pass streams annotated frames to OpenCV/FFmpeg without retaining rendered frames. FFmpeg atomically publishes `output/analyzed.mp4`; dimensions, frame count, and duration are verified before `result.json` and job success. Routes resolve only database-owned relative paths beneath `DATA_ROOT`.
